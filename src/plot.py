...
if plot_please == 131:
    ...
    colors = ['blue', 'red', 'orange', 'green', 'black', 'magenta', 'orange', 'c', 'gray',]
    fig, ax = plt.subplots(len(keys), 2)
    for t in range(2):
...
#plot_please = 132
if plot_please == 132:
    print("Refactored 6x6 goathunt experiment.")
    names = ['wen_refactor_iql_goat_hunt_6x6_slope0.0_170918',
             'wen_refactor_iql_goat_hunt_6x6_slope0.25_170918',
             'wen_refactor_iql_goat_hunt_6x6_slope0.5_170918',
    ...
    for t in range(len(keys)):
        # Main plot
        plot_db_compare(names, legend=legend, keys=keys, refactored=True,
                        title='4 agents 1 Goat 1 Sheep in 6x6 Env. (IQL, 5x5)' if t==0 else None,
                        test=t==1, max_time=max_time, min_time=min_time,
                        colors=colors, longest_runs=0, ax=[ax[t, i] for i in range(len(keys))],
                        legend_pos=['upper right'], legend_plot=[False, t==1, True, False, False], **kwargs)
        # Plot horizontal helper lines
...
if plot_please == 133:
    ...
    names = ['wen_refactor_iql_goat_hunt_10x10_slope0.8_180918',
             'wen_refactor_qmix_goat_hunt_10x10_slope0.8_180918',
             'wen_refactor_icql_goat_hunt_10x10_slope0.8_180918',
    ...
    legend = ['IQL', 'QMIX (skip)', 'ICQL (0.5)', 'IQL (nopain)', 'QMIX (skip, nopain)', 'ICQL (0.5, nopain)']
    keys = ['return_mean', 'ep_length_mean']
    # single_keys = ['loss', 'td_error_abs', 'q_taken_mean', 'grad_norm']
    single_keys = []
    kwargs = {'pm_std': False, 'use_sem': True, 'plot_individuals': '', 'fill_in': False, 'bin_size': 100}
    max_time = 1E6
    min_time = 0  # int(3E6)
    colors = ['red', 'magenta', 'y', 'green', 'blue', 'black', 'orange', 'c']
    ...
    for t in range(len(keys)):
        # Main plot
        plot_db_compare(names, legend=legend, keys=keys, refactored=True,
                        title='4 agents (5x5) 1 goat 1 sheep' if t == 0 else 'in 10x10 mountain env. (p=0.8)',
                        test=t == 1, max_time=max_time, min_time=min_time,
...
if plot_please == 134:
    ...
    for t in range(len(keys)):
        # Main plot
        plot_db_compare(names, legend=legend, keys=keys, refactored=True,
                        title='4 agents (5x5) 1 goat 1 sheep' if t == 0 else 'in 10x10 mountain env. (nopain)',
                        test=t == 1, max_time=max_time, min_time=min_time,
                        colors=colors, longest_runs=0, ax=[ax[i, t] for i in range(len(keys))],
                        legend_pos=['upper right'], legend_plot=[False, t == 1, True, False, False], **kwargs)
        # Plot horizontal helper lines
...
if plot_please == 135:
    ...
    names = ['wen_refactor_iql_goat_hunt_10x10_slope0.8_nopain_180918',
             ...
             'wen_refactor_icql_goat_hunt_10x10_slope0.8_nopain_180918',
             'wen_refactor_icql_goat_hunt_10x10_slope0.8_nopain_tdlam0.8_190918',
             'wen_refactor_iql_goat_hunt_10x10_slope0.8_nopain_walls_200918',
             'wen_refactor_qmix_goat_hunt_10x10_slope0.8_nopain_walls_200918',
             'wen_refactor_icql_goat_hunt_10x10_slope0.8_nopain_walls_lam0.0_200918',
             'wen_refactor_icql_goat_hunt_10x10_slope0.8_nopain_walls_lam0.8_200918']
    legend = ['IQL (no walls)', 'QMIX (skip, no walls)', 'ICQL (0.5, lam=0.0, no walls)', 'ICQL (0.5, lam=0.8, no walls)',
              'IQL (walls)', 'QMIX (skip, walls)', 'ICQL (0.5, lam=0.0, walls)', 'ICQL (0.5, lam=0.8, walls)']
    keys = ['return_mean', 'ep_length_mean']
    # single_keys = ['loss', 'td_error_abs', 'q_taken_mean', 'grad_norm']
    single_keys = []
    kwargs = {'pm_std': False, 'use_sem': True, 'plot_individuals': '', 'fill_in': False, 'bin_size': 100}
...
#plot_please = 142
if plot_please == 142:
    print("Starcraft 2 with many many agents")
...
#plot_please = 143
if plot_please == 143:
    print("Starcraft 1 with many many agents (this time for real)")
    legend = ['3m', '5m', '8m', '12m', '30m']
    names = ['SC1KK_3m', 'SC1KK_5m', 'SC1KK_8m', 'SC1KK_12m', 'SC2KK_30m']
    #names = ['SC1TEST_12m']
    keys = ['battle_won_mean']
    kwargs = {'pm_std': False, 'use_sem': True, 'plot_individuals': '', 'fill_in': True, 'bin_size': 100}
    max_time = 2.5E6
...
#plot_please = 148
if plot_please == 148:
    print("Paid experiment on refactored 10x10 nopain stag-only-hunt.")
    legend = ['Central-V', 'PAID (beta=0.0)', 'PAID (beta=0.05)', 'PAID (beta=0.1)', 'PAID (beta=0.25)']
    names = ['wen_refactor_centralV_stag_hunt_10x10_nopain_aftersoft_011118',
             'wen_refactor_paid_stag_hunt_10x10_nopain_aftersoft_beta0.0_prior1.0_011118',
             'wen_refactor_paid_stag_hunt_10x10_nopain_aftersoft_beta0.05_prior1.0_011118',
             'wen_refactor_paid_stag_hunt_10x10_nopain_aftersoft_beta0.1_prior1.0_011118',
             'wen_refactor_paid_stag_hunt_10x10_nopain_aftersoft_beta0.25_prior1.0_011118']
    keys = ['return_mean', 'ep_length_mean']
    kwargs = {'pm_std': False, 'use_sem': True, 'plot_individuals': '', 'fill_in': False, 'bin_size': 100}
    max_time = 10E10
    min_time = 6E6  # int(3E6)
    #colors = ['y', 'orange', 'red', 'green', 'c', 'blue']
    colors = ['black', 'green', 'lime', 'c', 'blue', 'magenta', 'red', 'orange', 'y']
    reward_horizons = [0, 5, 10]
    ep_length_horizons = []  # [15, 20, 25, 30, 40, 50]
    ...
    for t in range(2):
        plot_db_compare(names, keys=keys, refactored=True,
                        title='4 agents(5x5) on stag hunt' if t == 0 else 'in a 10x10 bounded gridworld',
                        test=t == 1, max_time=max_time, min_time=min_time,
                        colors=colors, longest_runs=0, ax=ax[:, t], legend=legend if t == 1 else None,
                        legend_pos=['upper right'], legend_plot=[False, t==1], **kwargs)
        for k in range(len(keys)):
...
#plot_please = 151
if plot_please == 151:
    print("Parameter exploration for IQL the goat_hunt with mountain_spawn, but rows and columns are sitched!")
    title = "IQL on goat hunt that spawn left and right"
...
#plot_please = 152
if plot_please == 152:
    print("Parameter exploration for IQL the goat_hunt with mountain_spawn, with correct rows and columns (I hope)!")
    title = "IQL on goat hunt in preferred habitats"
    legend = ['slope=0.0', 'slope=0.25', 'slope=0.5', 'slope=0.75']
    names = ['wen_refactor_iql_goat_hunt_slope0.0_v2_121218',
             'wen_refactor_iql_goat_hunt_slope0.25_v2_121218',
             'wen_refactor_iql_goat_hunt_slope0.5_v2_121218',
             'wen_refactor_iql_goat_hunt_slope0.75_v2_121218']
    keys = ['return_mean', 'ep_length_mean']
    kwargs = {'pm_std': False, 'use_sem': True, 'plot_individuals': '', 'fill_in': False, 'bin_size': 100}
    max_time = 10E6
    min_time = 0E6  # int(3E6)
    #colors = ['y', 'orange', 'red', 'green', 'c', 'blue']
    colors = ['black', 'green', 'blue', 'red', 'orange', 'y']
    reward_horizons = [0, 5, 10]
    ep_length_horizons = []  # [15, 20, 25, 30, 40, 50]
    fig, ax = plt.subplots(len(keys), 2)
    # Plot keys and their test
...
#plot_please = 153
if plot_please == 153:
    print("Parameter exploration for IQL the goat_hunt with mountain_spawn, and agents spawn in row directly over sheep!")
    title = "IQL on goat hunt in preferred habitats (h1)"
    legend = ['slope=0.0', 'slope=0.2', 'slope=0.4', 'slope=0.6', 'slope=0.8']
    names = ['wen_refactor_iql_goat_hunt_spawn_ar1_slope0.0_141218',
             'wen_refactor_iql_goat_hunt_spawn_ar1_slope0.2_141218',
             'wen_refactor_iql_goat_hunt_spawn_ar1_slope0.4_141218',
             'wen_refactor_iql_goat_hunt_spawn_ar1_slope0.6_141218',
             'wen_refactor_iql_goat_hunt_spawn_ar1_slope0.8_141218']
    keys = ['return_mean', 'ep_length_mean']
    kwargs = {'pm_std': False, 'use_sem': True, 'plot_individuals': '', 'fill_in': False, 'bin_size': 100}
    max_time = 10E6
    min_time = 0E6  # int(3E6)
...
if plot_please == 154:
    print("Parameter exploration for IQL the goat_hunt with mountain_spawn, and agents spawn in row directly over sheep!")
    title = "IQL on goat hunt in preferred habitats (h4)"
    legend = ['slope=0.0', 'slope=0.2', 'slope=0.4', 'slope=0.6', 'slope=0.8']
    names = ['wen_refactor_iql_goat_hunt_spawn_ar5_slope0.0_141218',
             'wen_refactor_iql_goat_hunt_spawn_ar5_slope0.2_141218',
             'wen_refactor_iql_goat_hunt_spawn_ar5_slope0.4_141218',
             'wen_refactor_iql_goat_hunt_spawn_ar5_slope0.6_141218',
             'wen_refactor_iql_goat_hunt_spawn_ar5_slope0.8_141218']
    keys = ['return_mean', 'ep_length_mean']
    kwargs = {'pm_std': False, 'use_sem': True, 'plot_individuals': '', 'fill_in': False, 'bin_size': 100}
    max_time = 10E6
    min_time = 0E6  # int(3E6)
...
if plot_please == 155:
    ...
    legend = ['Central-V', 'MACKRL']
    names = [#['F4I_FLOUNDERL_2s3z__partconst_1', 'F4I_FLOUNDERL_2s3z__partconst_2', 'F4I_FLOUNDERL_2s3z__partconst_3'],
              ['F4I_centralv_micro_baneling', 'F4I_FLOUNDERL_micro_baneling']]
    titles = [#'2s3z',
              'micro baneling']
    keys = ['Win rate']
    kwargs = {'pm_std': False, 'use_sem': True, 'plot_individuals': '', 'fill_in': True, 'bin_size': 100}
